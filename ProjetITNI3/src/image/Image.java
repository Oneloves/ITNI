...
// Initialisation
	    	...
	    	test3 = new BufferedImage(m_width, m_height, m_type);	
	    	test4 = new BufferedImage(m_width, m_height, m_type);	    	
...
// Some treatement 	
	    	...
	    	int[] m_imageArrayColorSelectionLeftBlue2 = m_imageArrayColorSelectionLeftBlue.clone();
	    	...
	    	arrayToImage(m_imageArrayColorSelectionMiddleLeftWithe, test);
	    	arrayToImage(m_imageArraySobelMiddleLeftWithe, test2);
	    	arrayToImage(m_imageArrayMiddleLeft, test3);
	    	arrayToImage(m_imageArrayColorSelectionLeftBlue2, test4);
// Array to Image
	    	arrayToImage();
	...
	public void sobel() {
		...
		for (int raw = 1; raw<m_height-1; raw++) {
		    for (int col = 1; col<m_width/2-1; col++) {		    				    
			    
		    	// Middle left cut  
		        m_imageArraySobelMiddleLeftWithe[raw*m_width+col] = 0;  
		    	if(m_imageArrayColorSelectionMiddleLeftWithe[(raw-1)*m_width+col-1] == 255) {	
				    pixelHorizontal = 0;
			    	PixelVertical =	
			    			-1 * m_imageArrayMiddleLeft[(raw-1)*m_width+col-1].m_gray + 1 * m_imageArrayMiddleLeft[(raw+1)*m_width+col-1].m_gray +
			    			-2 * m_imageArrayMiddleLeft[(raw-1)*m_width+col].m_gray   + 2 * m_imageArrayMiddleLeft[(raw+1)*m_width+col].m_gray   +
							-1 * m_imageArrayMiddleLeft[(raw-1)*m_width+col+1].m_gray + 1 * m_imageArrayMiddleLeft[(raw+1)*m_width+col+1].m_gray;		    	
		        ...
		        // Middle right cut	 
	    		m_imageArraySobelMiddleRightWithe[raw*m_width+col] = 0;
		    	if(m_imageArrayColorSelectionMiddleRightWithe[(raw-1)*m_width+col-1] == 255) {	   	
			    	pixelHorizontal = 0;
			    	PixelVertical =	
			    			-1 * m_imageArrayMiddleRight[(raw-1)*m_width+col-1].m_gray + 1 * m_imageArrayMiddleRight[(raw+1)*m_width+col-1].m_gray +
			    			-2 * m_imageArrayMiddleRight[(raw-1)*m_width+col].m_gray   + 2 * m_imageArrayMiddleRight[(raw+1)*m_width+col].m_gray   +
							-1 * m_imageArrayMiddleRight[(raw-1)*m_width+col+1].m_gray + 1 * m_imageArrayMiddleRight[(raw+1)*m_width+col+1].m_gray;		    			    	
			    	pixel = (int) Math.sqrt(pixelHorizontal * pixelHorizontal + PixelVertical * PixelVertical);
			    	if (pixel > 40)
			    		pixel  = 255;		        
		        ...
		        // Left Blue cut
		        m_imageArraySobelLeftBlue[raw*m_width+col] = 0;  
			    	if(m_imageArrayColorSelectionLeftBlue[(raw-1)*m_width+col-1] == 255) {	
			    	pixelHorizontal = 	
			    			 1 * m_imageArrayLeft[(raw-1)*m_width+col-1].m_gray  +  2 * m_imageArrayLeft[raw*m_width+col-1].m_gray +  1 * m_imageArrayLeft[(raw+1)*m_width+col-1].m_gray +		    			
			    			-1 * m_imageArrayLeft[(raw-1)*m_width+col+1].m_gray  -  2 * m_imageArrayLeft[raw*m_width+col+1].m_gray -  1 * m_imageArrayLeft[(raw+1)*m_width+col+1].m_gray;	
			    	PixelVertical =	
			    			-1 * m_imageArrayLeft[(raw-1)*m_width+col-1].m_gray + 1 * m_imageArrayLeft[(raw+1)*m_width+col-1].m_gray +
			    			-2 * m_imageArrayLeft[(raw-1)*m_width+col].m_gray   + 2 * m_imageArrayLeft[(raw+1)*m_width+col].m_gray   +
							-1 * m_imageArrayLeft[(raw-1)*m_width+col+1].m_gray + 1 * m_imageArrayLeft[(raw+1)*m_width+col+1].m_gray;		    	
			    	pixel = (int) Math.sqrt(pixelHorizontal * pixelHorizontal + PixelVertical * PixelVertical);
			    	if (pixel > 40)
			    		pixel  = 255;		        
		        ...
		        // Right Blue cut		
		        m_imageArraySobelRightBlue[raw*m_width+col] = 0;  
		    	if(m_imageArrayColorSelectionRightBlue[(raw-1)*m_width+col-1] == 255) {	
			    	pixelHorizontal = 	
			    			 1 * m_imageArrayRight[(raw-1)*m_width+col-1].m_gray  +  2 * m_imageArrayRight[raw*m_width+col-1].m_gray +  1 * m_imageArrayRight[(raw+1)*m_width+col-1].m_gray +		    			
			    			-1 * m_imageArrayRight[(raw-1)*m_width+col+1].m_gray  -  2 * m_imageArrayRight[raw*m_width+col+1].m_gray -  1 * m_imageArrayRight[(raw+1)*m_width+col+1].m_gray;	
			    	PixelVertical =	
			    			-1 * m_imageArrayRight[(raw-1)*m_width+col-1].m_gray + 1 * m_imageArrayRight[(raw+1)*m_width+col-1].m_gray +
			    			-2 * m_imageArrayRight[(raw-1)*m_width+col].m_gray   + 2 * m_imageArrayRight[(raw+1)*m_width+col].m_gray   +
							-1 * m_imageArrayRight[(raw-1)*m_width+col+1].m_gray + 1 * m_imageArrayRight[(raw+1)*m_width+col+1].m_gray;		    	
			    	pixel = (int) Math.sqrt(pixelHorizontal * pixelHorizontal + PixelVertical * PixelVertical);
			    	if (pixel > 40)
			    		pixel  = 255;		        
		        ...
		        // Left Red cut		
		        m_imageArraySobelLeftRed[raw*m_width+col] = 0;  
		    	if(m_imageArrayColorSelectionLeftRed[(raw-1)*m_width+col-1] == 255) {	
			    	pixelHorizontal = 	
			    			 1 * m_imageArrayLeft[(raw-1)*m_width+col-1].m_gray  +  2 * m_imageArrayLeft[raw*m_width+col-1].m_gray +  1 * m_imageArrayLeft[(raw+1)*m_width+col-1].m_gray +		    			
			    			-1 * m_imageArrayLeft[(raw-1)*m_width+col+1].m_gray  -  2 * m_imageArrayLeft[raw*m_width+col+1].m_gray -  1 * m_imageArrayLeft[(raw+1)*m_width+col+1].m_gray;	
			    	PixelVertical =	
		        ...
		        // Right Red cut		
		        m_imageArraySobelRightRed[raw*m_width+col] = 0;	  
		    	if(m_imageArrayColorSelectionRightRed[(raw-1)*m_width+col-1] == 255) {	
			    	pixelHorizontal = 	
			    			 1 * m_imageArrayRight[(raw-1)*m_width+col-1].m_gray  +  2 * m_imageArrayRight[raw*m_width+col-1].m_gray +  1 * m_imageArrayRight[(raw+1)*m_width+col-1].m_gray +		    			
			    			-1 * m_imageArrayRight[(raw-1)*m_width+col+1].m_gray  -  2 * m_imageArrayRight[raw*m_width+col+1].m_gray -  1 * m_imageArrayRight[(raw+1)*m_width+col+1].m_gray;	
			    	PixelVertical =	
	...
	public void cutImage() {
		...
		for (int raw = 0; raw<m_height; raw++) {
		    for (int col = 0; col<m_width; col++) {	
				...
				if(d<0 || col>m_width/2 || raw<m_height/2) {
